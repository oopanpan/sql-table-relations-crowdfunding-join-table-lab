...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_title
  "SELECT title, SUM(pledges.amount) FROM projects INNER JOIN pledges WHERE projects.id = pledges.project_id GROUP BY projects.title ORDER BY projects.title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT name, age, SUM(pledges.amount) FROM users INNER JOIN pledges WHERE users.id = pledges.user_id GROUP BY name ORDER BY name;"
end
...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) AS total_funded
